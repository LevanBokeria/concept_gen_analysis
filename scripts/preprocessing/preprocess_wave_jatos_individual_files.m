function preprocess_wave_jatos_individual_files()

%% Description

% This script will preprocess the newest wave data from JATOS with
% individual files that are auto exported.

% preprocess_wave, which takes one big file containing all
% the wave data including all the components of all the participants of the
% wave. And then looks at who has a "data_submission" module.

% This script, instead looks at the individual JATOS auto exports, which
% for some reason never contain the "data_submission" module, but the last
% "transition" module has all the data. So this script looks at that.
...
all_wave_files = lsDir(fullfile(home,'data','jatos_individual_exports',...
    batch_wave_name),{'jatos'});

nPtp = numel(all_wave_files);
...
for iPtp = 1:nPtp
    ...
    % Read the file
    curr_data_table = import_and_parse_jatos_individual_file(all_wave_files{iPtp});
    
%     curr_data_table = readtable(all_wave_files{iPtp},'FileType','text');
    curr_cell = curr_data_table.Data{height(curr_data_table)};
    
    % First, determine if this participant has the transition component
    if strfind(curr_cell,'[transition_start---')
        
        % Get just the contents
        [split1,~] = split(curr_cell,'[transition_start---');
        [split2,~] = split(split1{2},'---transition_end]');
        
        decoded = jsondecode(split2{1});
        
        % Did they complete debriefing? So ds was sent
        ...
        % Still, decode this last element
        splitter = '---';
        components_split = split(curr_cell,splitter);
        decoded = jsondecode(components_split{2});
