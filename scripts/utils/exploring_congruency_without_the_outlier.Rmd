...
resultstable_all_ptp_analyzed <- 
        outList$resultstable_all_ptp_analyzed
resultstable_qc_pass_ptp_analyzed <- 
    outList$resultstable_qc_pass_ptp_analyzed
resultstable_qc_pass_ptp_analyzed_gathered <- 
    outList$resultstable_qc_pass_ptp_analyzed_gathered
...
st_no_outlier <- 
        ...
        droplevels() %>%
        filter(case_when(
                experiment == 1 ~ prompt_img_name != 'Sledge',
                experiment == 2 ~ prompt_img_name != 'Gingerman'
                )
               ) %>%
        group_by(experiment,
                 congruency,
...
st_long_mean_type <-
        st %>%
        select(-c(n_all_data,n_no_outlier)) %>%
...
st_ses_1_2_mean <- 
        ...
        group_by(experiment,
                 ...
                 phase,
                 mean_type) %>%
        summarise(ses_1_2_mean = mean(mean_accuracy)) %>%
        ungroup()
...
st_by_target_ses_1_2_mean <-
        ...
        group_by(experiment,
                 ...
                 phase,
                 prompt_img_name) %>%
        summarise(ses_1_2_mean = mean(mean))
```

```{r congruency-plots, fig.height=8, fig.width=6}
...
st_phase_diff_long %>%
        ...
        stat_summary(fun=mean, 
                     ...
                     color="blue",
                     fill="blue") +        
        facet_grid(mean_type~experiment,
                   labeller = label_both)
...
#                      color="blue",
#                      fill="blue") +        
#         facet_grid(prompt_img_name~experiment,
#                    labeller = label_both)

```

```{r congruency-by-arr-plots, fig.height=6, fig.width=8}
...
#                      color="blue",
#                      fill="blue") +        
#         facet_grid(prompt_img_name~arr_phase_1_name,
#                    labeller = label_both)
...
st_ses_1_2_avg <-
        ...
        group_by(experiment,
                 ...
                 phase) %>%
        summarise(
                across(.cols = c(mean_all_data,
                                 mean_no_outlier),
               .fns = mean,
               .names = c('ses_1_2_{.col}'))
               ) %>%
        ungroup()

st_ses_1_2_avg_wide_phase <-
        st_ses_1_2_avg %>%
        pivot_wider(names_from = phase,
                    names_prefix = 'phase_',
                    values_from = c(
                            ses_1_2_mean_all_data,
                            ses_1_2_mean_no_outlier
                    )) %>%
        mutate(phase_2_min_phase_1_ses_1_2_mean_all_data = 
                       ses_1_2_mean_all_data_phase_2 - 
                       ses_1_2_mean_all_data_phase_1,
               phase_2_min_phase_1_ses_1_2_mean_no_outlier = 
                       ses_1_2_mean_no_outlier_phase_2 - 
                       ses_1_2_mean_no_outlier_phase_1)

st_ses_1_2_avg_long_phase <-
        st_ses_1_2_avg_wide_phase %>%
        pivot_longer()

ses_1_2_perf_table_wide <- 
        ses_1_2_perf_table_wide %>%
        mutate(phase_2_min_phase_1 = 
                       phase_2 - phase_1)

ses_1_2_perf_table_long <-
        ses_1_2_perf_table_wide %>%
        pivot_longer(cols = c(phase_1,
                              phase_2,
                              phase_2_min_phase_1),
                     namesto = 'var_type',
                     valuesto = 'ses_1_2_perf')

```

```{r plot-this, fig.width=6, fig.height=4}

ses_1_2_perf_table_long %>%
        filter(var_type == 'phase_2_min_phase_1') %>%
        ggplot(aes(x=congruency,y=ses_1_2_perf)) +
        geom_boxplot(notch = TRUE,
                     width = 0.3) +
...
```{r plot-by-phase, fig.width=10, fig.height=4}

ses_1_2_perf_table_long %>%
        filter(var_type != 'phase_2_min_phase_1') %>%
        ggplot(aes(x=var_type,y=ses_1_2_perf,fill=var_type)) +
        geom_boxplot(notch = TRUE,
                     width = 0.3) +
        geom_point(alpha = 0.2) +
...
data_arr_1_c1 <- resultstable_qc_pass_ptp_analyzed %>%
    filter(arr_phase_1_name == 1 & congruency == 1) %>%
    select(phase_2_min_phase_1_ses_1_2_perf) %>% .[[1]]
data_arr_1_c0 <- resultstable_qc_pass_ptp_analyzed %>%
    filter(arr_phase_1_name == 1 & congruency == 0) %>%
    select(phase_2_min_phase_1_ses_1_2_perf) %>% .[[1]]
data_arr_2_c1 <- resultstable_qc_pass_ptp_analyzed %>%
    filter(arr_phase_1_name == 2 & congruency == 1) %>%
    select(phase_2_min_phase_1_ses_1_2_perf) %>% .[[1]]
data_arr_2_c0 <- resultstable_qc_pass_ptp_analyzed %>%
...
model  <- resultstable_qc_pass_ptp_analyzed %>%
        filter(experiment == which_expt) %>%
...
aov_3_expt_1 <- resultstable_qc_pass_ptp_analyzed %>%
        filter(experiment == which_expt) %>%
        anova_test(
...
pwc <- resultstable_qc_pass_ptp_analyzed %>%
        filter(experiment == which_expt) %>%
    group_by(arr_phase_1_name) %>%
  emmeanstest(phase_2_min_phase_1_ses_1_2_perf ~ congruency, p.adjust.method = "bonferroni") 
pwc %>%
    knitr::kable(caption = 'Expt1: congruency -X- concept -X- arrangement') %>%
    kable_styling(bootstrap_options = "striped")
...
data_for_bayes_anova <- resultstable_qc_pass_ptp_analyzed %>% 
        filter(experiment == which_expt) %>%
        rename(
                Cg = congruency,
                Cc = concept_phase_1,
...
data_arr_3_c1 <- resultstable_qc_pass_ptp_analyzed %>%
    filter(arr_phase_1_name == 3 & congruency == 1) %>%
    select(phase_2_min_phase_1_ses_1_2_perf) %>% .[[1]]
data_arr_3_c0 <- resultstable_qc_pass_ptp_analyzed %>%
    filter(arr_phase_1_name == 3 & congruency == 0) %>%
    select(phase_2_min_phase_1_ses_1_2_perf) %>% .[[1]]
data_arr_4_c1 <- resultstable_qc_pass_ptp_analyzed %>%
    filter(arr_phase_1_name == 4 & congruency == 1) %>%
    select(phase_2_min_phase_1_ses_1_2_perf) %>% .[[1]]
data_arr_4_c0 <- resultstable_qc_pass_ptp_analyzed %>%
...
aov_3_expt_2 <- resultstable_qc_pass_ptp_analyzed %>%
        filter(experiment == which_expt) %>%
        anova_test(
        phase_2_min_phase_1_ses_1_2_perf ~
                congruency*concept_phase_1*arr_phase_1_name)
