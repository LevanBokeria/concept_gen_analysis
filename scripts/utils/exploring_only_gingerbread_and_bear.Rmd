...
# We have to exclude the sledge, and recalculate all the variables in the results_table_all_ptp_analyzed


# Plan is to recalculate phase 2 ses 1-2 for each participant, and then substitute that new data in the existing tables

summary_table <- 
        long_form_data_qc_pass_ptp_analyzed %>%
        filter(experiment == which_expt,
               session == c(1,2)) %>%
        filter(prompt_img_name != 'Sledge') %>% 
        group_by(congruency,ptp,phase,session) %>%
        get_summary_stats(correct,type='mean')

ses_1_2_perf_table <- 
        summary_table %>%
        group_by(congruency,ptp,phase) %>%
        get_summary_stats(mean,type='mean') %>%
        select(-c(variable,n)) %>%
        rename(session_1_2_perf = mean)
        
ses_1_2_perf_table_wide <-
        ses_1_2_perf_table %>%
        pivot_wider(names_from = phase,
                    names_prefix = 'phase_',
                    values_from = session_1_2_perf)
