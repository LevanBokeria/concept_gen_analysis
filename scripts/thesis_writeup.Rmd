...
p <- results_table_qc_pass_ptp_analyzed %>%
...
ggsave(filename = 'expt_1_congruency.png', plot = p, path = '../results/plots/', width = 3, height = 4, device='png', dpi=300)
...
# Effect size for the block differences
...
p<-results_table_qc_pass_ptp_analyzed_gathered %>% 
        filter(experiment == which_expt) %>%
        ggplot(aes(x=phase,
                   y=ses_1_2_perf,
                   fill=phase)) +
        ...
        facet_wrap(~congruency, 
                   labeller = labeller(congruency = c("0" = "Incongruent",
                                                         "1" = "Congruent")))
print(p)
ggsave(filename = 'expt_1_congruency_by_phase.png', plot = p, path = '../results/plots/', width = 5, height = 4, device='png', dpi=300)
...
ggsave(filename = 'expt_1_congruency_by_arr.png', plot = p, path = '../results/plots/', width = 5, height = 4, device='png', dpi=300)
...
ggsave(filename = 'expt_1_congruency_by_phase_by_arr.png', plot = p, path = '../results/plots/', width = 6, height = 4, device='png', dpi=300)
...
# Filter to have only blocks 1-2, only incongruent, phase, arrangement, and 
# the target that was the prompt for the trial

p <- long_form_data_qc_pass_ptp_analyzed %>%
        filter(experiment == which_expt,
               (session %in% c(1,2))) %>%
        select(congruency,
               arr_phase_1_name,
...
ggsave(filename = 'expt_1_performance_by_toy.png', plot = p, path = '../results/plots/', width = 12, height = 5, device='png', dpi=300)
...
ggsave(filename = 'expt_2_congruency.png', plot = p, path = '../results/plots/', width = 3, height = 4, device='png', dpi=300)
...
print(p)
ggsave(filename = 'expt_2_congruency_by_phase.png', plot = p, path = '../results/plots/', width = 5, height = 4, device='png', dpi=300)
...
results_table_qc_pass_ptp_analyzed_gathered %>% 
        ...
        geom_point(pch=21,
                   position = position_jitterdodge(dodge.width=0.5,
                                                   jitter.width=0.05)) +
        ylab("Percent correct in blocks 1&2") + 
        theme(axis.text.x = element_text(size=10),
              axis.text.y = element_text(size=10),
              axis.title.x=element_blank(),
              legend.position = 'top') + 
...
ggsave(filename = 'expt_2_congruency_by_arr.png', plot = p, path = '../results/plots/', width = 5, height = 4, device='png', dpi=300)
...
ggsave(filename = 'expt_2_congruency_by_phase_by_arr.png', plot = p, path = '../results/plots/', width = 6, height = 4, device='png', dpi=300)
...
long_form_data_qc_pass_ptp_analyzed %>%
        ...
        ylab('% Correct blocks 1&2') + 
        xlab('Arrangement in this phase')
        
print(p)

ggsave(filename = 'expt_2_performance_by_toy.png', plot = p, path = '../results/plots/', width = 12, height = 5, device='png', dpi=300)
```
...
# Filter to have only blocks 1-2, only incongruent, phase, arrangement, and the target that was the prompt for the trial
...
# Just phase 1 data for blocks 1 and 2, by arrangement order
results_table_qc_pass_ptp_analyzed %>%
